//! Deterministic instruction-count harness for cold-start recovery:
//! `Db::open()` after a simulated crash (an in-flight, unflushed batch of
//! WAL records that must be replayed into a fresh memtable), on top of
//! manifest state from three prior successful flushes.
//!
//! Structure: warm up like `write_path.rs` (3 put batches, each flushed),
//! then one more batch of puts that is *never* flushed or acked-closed —
//! `into_device()` + a fresh `Db` + `open()` simulates the crash/restart,
//! and that `open()` call is what this profiles: manifest recovery, the
//! open-time free-list sweep, and WAL replay of the unflushed batch.
//!
...
const FLUSHED_BATCHES: usize = 3;
/// The in-flight batch: written and WAL-acked, never flushed. `open()` must
...
fn main() {
    ...
    for batch in 0..FLUSHED_BATCHES {
        for i in 0..PUT_BATCH {
            let klen = rng.range(8, 32);
            make_key(&mut kbuf, &mut rng, klen);
            // `% 26` bounds the addend to `[0, 26)`; `batch < FLUSHED_BATCHES
    ...
    // The crash: this batch is durably WAL-acked but never flushed.
    for i in 0..UNFLUSHED_BATCH {
        let klen = rng.range(8, 32);
        make_key(&mut kbuf, &mut rng, klen);
        kbuf[0] = b'z';
        // `% 26` bounds the addend to `[0, 26)`; the `u8` cast cannot truncate.
        #[allow(clippy::cast_possible_truncation)]
        {
            kbuf[1] = b'a' + (i % 26) as u8;
        }
        let vlen = rng.range(16, 200);
        make_val(&mut vbuf, &mut rng, vlen);
        block_on(db.put(&kbuf[..klen], &vbuf[..vlen])).expect("unflushed put");
    }

    // Simulate the crash/restart: same device, fresh `Db`, fresh `open()`.
    // This is the call this harness profiles.
    let device = db.into_device();
    let mut db2 = BenchDb::new(device, config());
    let report = block_on(db2.open()).expect("recovery open");
    println!("recovered_records={}", report.recovered_records);
}
