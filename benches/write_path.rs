//! Deterministic instruction-count harness for the write path (`Db::put` /
//! `Db::delete`), profiled with `valgrind --tool=callgrind` / `cachegrind`.
//!
...
//!
//! Workload: a realistic mixed put/get/delete sequence through the public
//! `Db` API, driven by an in-memory `BlockDevice` (the same style used by the
//! integration tests) and a fixed-seed PRNG for reproducible key/value
//! distributions. Every run with the same binary produces the same
//! instruction count — that is the point.
...
/// 64 KiB-arena memtable cap (klen 8-32 + vlen 16-200 + slot overhead
...
const DELETES: usize = 200; // post-4th-flush, stays in the (unflushable) memtable
const GETS: usize = 4000;
...
fn main() {
    ...
    // Fresh unique-key inserts in batches, flushed after each so the table
    // region sees real SSTable traffic too.
    ...
    // Update pass: overwrite existing keys (exercises the duplicate-key
    // planning branches, in-place and reuse-dead alike), then the 4th and
    // final flush (L0 is now full; no more flushes are possible in v0.3).
    for i in 0..UPDATES {
    ...
    // Delete pass: stays in the memtable (L0 cannot take a 5th table).
    for i in 0..DELETES {
        let key = &written[(i * 7) % written.len()];
        block_on(db.delete(key)).expect("delete");
    }

    // Read pass: mixed hits (recently written / updated keys, some now
    // shadowed by deletes) and misses (keys never written).
    ...
    // Touch the result so none of this is optimized away, and print a
    // human-sanity-check line (not part of the measurement: instruction
    // counts come from valgrind, not from anything this process prints).
